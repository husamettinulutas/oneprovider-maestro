...
export class CodexIntegration implements AgentIntegration {
  ...
  async getStatus(): Promise<IntegrationStatus> {
    ...
    if (content !== undefined) {
      ...
      active =
        findProviderSection(content) !== undefined &&
  ...
  async apply(
    ...
    if (!this.globalState.get<CodexSnapshot>(SNAPSHOT_KEY)) {
      ...
      const snapshot: CodexSnapshot = {
        configPath,
        previousModelLine: existing ? findTopLevelLine(existing, 'model') : undefined,
        previousProviderLine: existing
          ? findTopLevelLine(existing, 'model_provider')
          : undefined,
        ...
        previousShowRawReasoningLine: existing
          ? findTopLevelLine(existing, 'show_raw_agent_reasoning')
          : undefined,
        previousReasoningEffortLine: existing
          ? findTopLevelLine(existing, 'model_reasoning_effort')
          : undefined,
        previousReasoningSummaryLine: existing
          ? findTopLevelLine(existing, 'model_reasoning_summary')
          : undefined,
        previousUserEnvKey:
    ...
    let content = stripMarkerLines(existing ?? '');
  ...
  async restore(): Promise<IntegrationStatus> {
    ...
    if (existing !== undefined) {
      /*
        Rebuild the pre-Maestro file from the snapshot and hand it to the same
        function the uninstall hook calls, so the two paths cannot drift apart.
        clearCodexConfig only reads top-level assignments and the provider
        section out of it, so the managed lines first and the section after is
        all the shape it needs.
      */
      const priorLines = [
      ...
      const previous =
        priorLines.length > 0 || snapshot?.previousProviderSection
          ? `${priorLines.join('\n')}\n${snapshot?.previousProviderSection ?? ''}`
          : undefined;

      writeTextFileAtomic(configPath, clearCodexConfig(existing, previous));
